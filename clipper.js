...
(function ()
{
   ...
   */
    if (use_xyz)
    {
        ...
        {
            this.X = dp.X; this.Y = dp.Y; this.Z = 0;
        };
    ...
    //ClipperLib.ClipperBase.tolerance = 0.00000001;
    //ClipperLib.ClipperBase.toleranceSqrt = 0.0001;
    ...
    ClipperLib.ClipperBase.prototype.AddPath = function (pg, polyType, Closed)
    {
        ...
        for (;;)
        {
            ...
            if (E.Prev == E.Next)
                break;
            else if (Closed && ClipperLib.ClipperBase.SlopesEqual(E.Prev.Curr, E.Curr, E.Next.Curr)
    ...
    ClipperLib.Clipper.prototype.AddLocalMinPoly = function (e1, e2, pt)
    {
        ...
        if (prevE !== null && prevE.OutIdx >= 0 && (ClipperLib.Clipper.TopX(prevE, pt.Y) == ClipperLib.Clipper.TopX(e, pt.Y))
            && ClipperLib.ClipperBase.SlopesEqual(e, prevE) && (e.WindDelta !== 0) && (prevE.WindDelta !== 0))
        {
   ...
   */
    ...
    ClipperLib.Clipper.prototype.IntersectPoint = function (edge1, edge2, ip)
    {
        ...
        else if (edge1.Delta.X === 0)
        {
            ...
            else
            {
                ...
                ip.Value.Y = ip.Value.X / edge2.Dx + b2;
            }
        }
        else if (edge2.Delta.X === 0)
        {
            ...
            else
            {
                ...
                ip.Value.Y = ip.Value.X / edge1.Dx + b1;
            }
        }
        else
        {
            ...
            if (Math.abs(edge1.Dx) < Math.abs(edge2.Dx))
                ip.Value.X = edge1.Dx * q + b1;
            else
                ip.Value.X = edge2.Dx * q + b2;
        }
    ...
    ClipperLib.Clipper.prototype.JoinPoints = function (j, outRec1, outRec2)
    {
        ...
        else
        {
            ...
            !ClipperLib.ClipperBase.SlopesEqual(op1.Pt, op1b.Pt, j.OffPt));
            if (Reverse1)
            {
                op1b = op1.Prev;
                while ((ClipperLib.IntPoint.op_Equality(op1b.Pt, op1.Pt)) && (op1b != op1))
            ...
            if (Reverse2)
            {
                ...
                if ((op2b.Pt.Y > op2.Pt.Y) ||
                    !ClipperLib.ClipperBase.SlopesEqual(op2.Pt, op2b.Pt, j.OffPt))
                    return false;
            }
    ...
    ClipperLib.Clipper.UpdateBotPt = function (pt, botPt)
    {
        if (pt.Y > botPt.Value.Y || (pt.Y == botPt.Value.Y && pt.X < botPt.Value.X))
        {
            //botPt.Value = pt;
            botPt.Value.X = pt.X;
            botPt.Value.Y = pt.Y;
    ...
    ClipperLib.Clipper.StripDupsAndGetBotPt = function (in_path, out_path, closed, botPt)
    {
        botPt.Value = new ClipperLib.IntPoint(0, 0);
        var len = in_path.length;
        if (closed)
            while (len > 0 && (ClipperLib.IntPoint.op_Equality(in_path[0], in_path[len - 1])))
                len--;
        if (len === 0)
            return false;
        //out_path.set_Capacity(len);
        var j = 0;
        //out_path.push(in_path[0]);
        out_path.push(new ClipperLib.IntPoint(in_path[0]));
        //botPt.Value = in_path[0];
        botPt.Value.X = in_path[0].X;
        botPt.Value.Y = in_path[0].Y;
        for (var i = 1; i < len; ++i)
            if (ClipperLib.IntPoint.op_Inequality(in_path[i], out_path[j]))
            {
                //out_path.push(in_path[i]);
                out_path.push(new ClipperLib.IntPoint(in_path[i]));
                j++;
                if (out_path[j].Y > botPt.Value.Y ||
                    ((out_path[j].Y == botPt.Value.Y) && out_path[j].X < botPt.Value.X))
                //botPt.Value = out_path[j];
                    botPt.Value = new ClipperLib.IntPoint(out_path[j]);
            }
        j++;
        if (j < 2 || (closed && (j == 2)))
            j = 0;
        while (out_path.length > j)
            out_path.splice(j, 1);
        return j > 0;
    };
    ...
    if (use_deprecated)
    {
        ...
        ClipperLib.Clipper.OffsetPolygons = ClipperLib.Clipper.prototype.OffsetPolygons = function (poly, delta, jointype, MiterLimit, AutoFix)
        {
            ...
            // function (poly, delta, jointype, MiterLimit, AutoFix)
            if(alen == 5)
                return ClipperLib.Clipper.OffsetPaths(poly, delta, jointype, ClipperLib.EndType.etClosed, MiterLimit);

            // function (poly, delta, jointype, MiterLimit)
            else if (alen == 4)
                return ClipperLib.Clipper.OffsetPaths(poly, delta, jointype, ClipperLib.EndType.etClosed, MiterLimit);

            // function (poly, delta, jointype)
            else if (alen == 3)
                return ClipperLib.Clipper.OffsetPaths(polys, delta, jointype, ClipperLib.EndType.etClosed, 0);

            // function (polys, delta)
            else if (alen == 2)
                return ClipperLib.Clipper.OffsetPolygons(polys, delta, ClipperLib.JoinType.jtSquare, 0, true);
        }
        ...
        ClipperLib.Clipper.PolyTreeToPolygons = function (polytree, polys)
        {
            ClipperLib.Clear(polys);
            // polys.set_Capacity(polytree.Total());
            ClipperLib.Clipper.AddPolyNodeToPaths(polytree, ClipperLib.Clipper.NodeType.ntAny, polys);
        };
    }
    ...
    ClipperLib.Clipper.AddOuterPolyNodeToExPolygons = function(polynode, expolygons)
    {
        ...
        for (i = 0; i < ilen; i++ )
        {
            ...
            {
                n = childs2[j];
                ClipperLib.Clipper.AddOuterPolyNodeToExPolygons(n, expolygons);
            }
        }
        expolygons.push(ep);
    };

    ClipperLib.Clipper.PolyTreeToExPolygons = function(polytree, expolygons)
    {
        ClipperLib.Clear(expolygons);
        var node, i, childs, ilen;
        for (i = 0, childs = polytree.Childs(), ilen = childs.length; i < ilen; i++ )
        {
            node = childs[i];
            ClipperLib.Clipper.AddOuterPolyNodeToExPolygons(node, expolygons);
        }
    };

    ClipperLib.Clipper.ExPolygonsToPolygons = function(expolygons, polygons)
    {
        var a, i, alen, ilen;
        polygons.length = 0;
        for(a = 0, alen = expolygons.length; a < alen; a++)
        {
            polygons.push(expolygons[a].outer);
            for(i = 0, ilen = expolygons[a].holes.length; i < ilen; i++)
            {
                polygons.push(expolygons[a].holes[i]);
            }
        }
    }

    ClipperLib.Clipper.SimplifyPolygon = ClipperLib.Clipper.prototype.SimplifyPolygon = function (poly, fillType)
    {
        var result = new Array();
    ...
    ClipperLib.Clipper.SimplifyPolygons = ClipperLib.Clipper.prototype.SimplifyPolygons = function (polys, fillType)
    {
        var result = new Array();
        var c = new ClipperLib.Clipper(0);
        c.StrictlySimple = true;
    ...
    ClipperLib.ClipperOffset.prototype.AddPath = function (path, joinType, endType)
    {
        ...
        newNode.m_polygon.push(new ClipperLib.IntPoint(path[0].X, path[0].Y));
        var j = 0,
            k = 0;
        for (var i = 1; i <= highI; i++)
            ...
            {
                j++;
                newNode.m_polygon.push(new ClipperLib.IntPoint(path[i].X, path[i].Y));
    ...
    ClipperLib.ClipperOffset.GetUnitNormal = function (pt1, pt2)
    {
        ...
        var dy = (pt2.Y - pt1.Y);
        if ((dx == 0) && (dy == 0))
            return new ClipperLib.IntPoint(0, 0);
        var f = 1 / Math.sqrt(dx * dx + dy * dy);
        dx *= f;
        dy *= f;
        return new ClipperLib.IntPoint(dy, -dx);
    };
    ClipperLib.ClipperOffset.prototype.DoOffset = function (delta)
    {
        this.m_destPolys = new Array();
        this.m_delta = delta;
        this.absDelta = Math.abs(delta);


        //if Zero offset, just copy any CLOSED polygons to m_p and return ...
        if (this.absDelta < 1.0e-06) {
        ...
        else if (this.ArcTolerance > this.absDelta * ClipperLib.ClipperOffset.def_arc_tolerance)
            y = this.absDelta * ClipperLib.ClipperOffset.def_arc_tolerance;
        else
            y = this.ArcTolerance;
        //see offset_triginometry2.svg in the documentation folder ...
        var steps = 3.14159265358979 / Math.acos(1 - y / this.absDelta);
        ...
        for (var i = 0; i < this.m_polyNodes.ChildCount(); i++)
        {
            ...
            if (len == 1)
            {
                if (node.m_jointype == ClipperLib.JoinType.jtRound)
                {
                    ...
                    {
                        this.m_destPoly.push(new ClipperLib.IntPoint(this.m_srcPoly[0].X + X * delta, this.m_srcPoly[0].Y + Y * delta));
                        var X2 = X;
            ...
            if (node.m_endtype == ClipperLib.EndType.etClosedLine || node.m_endtype == ClipperLib.EndType.etClosedPolygon)
                ...
                this.m_normals.push(new ClipperLib.IntPoint(this.m_normals[len - 2]));
            if (node.m_endtype == ClipperLib.EndType.etClosedPolygon)
            {
                var k = len - 1;
                for (var j = 0; j < len; j++)
            ...
            else if (node.m_endtype == ClipperLib.EndType.etClosedLine)
            {
                ...
                for (var j = len - 1; j > 0; j--)
                    this.m_normals[j] = new ClipperLib.IntPoint(-this.m_normals[j - 1].X, -this.m_normals[j - 1].Y);
                this.m_normals[0] = new ClipperLib.IntPoint(-n.X, -n.Y);
                k = 0;
                for (var j = len - 1; j >= 0; j--)
                    k = this.OffsetPoint(j, k, node.m_jointype);
                this.m_destPolys.push(this.m_destPoly);
            ...
            else
            {
                ...
                {
                    var j = len - 1;
                    pt1 = new ClipperLib.IntPoint(this.m_srcPoly[j].X + this.m_normals[j].X * delta, this.m_srcPoly[j].Y + this.m_normals[j].Y * delta);
                ...
                else
                {
                    ...
                    k = len - 2;
                    this.m_sinA = 0;
                    this.m_normals[j] = new ClipperLib.IntPoint(-this.m_normals[j].X, -this.m_normals[j].Y);
                ...
                for (var j = len - 1; j > 0; j--)
                    this.m_normals[j] = new ClipperLib.IntPoint(-this.m_normals[j - 1].X, -this.m_normals[j - 1].Y);
                this.m_normals[0] = new ClipperLib.IntPoint(-this.m_normals[1].X, -this.m_normals[1].Y);
                k = len - 1;
                for (var j = k - 1; j > 0; --j)
                    k = this.OffsetPoint(j, k, node.m_jointype);
                if (node.m_endtype == ClipperLib.EndType.etOpenButt)
                {
                    pt1 = new ClipperLib.IntPoint(this.m_srcPoly[0].X - this.m_normals[0].X * delta, this.m_srcPoly[0].Y - this.m_normals[0].Y * delta);
                    this.m_destPoly.push(pt1);
                    pt1 = new ClipperLib.IntPoint(this.m_srcPoly[0].X + this.m_normals[0].X * delta, this.m_srcPoly[0].Y + this.m_normals[0].Y * delta);
                    this.m_destPoly.push(pt1);
                }
    ...
    ClipperLib.ClipperOffset.prototype.OffsetPoint = function (j, k, jointype)
    {
        ...
        {
            this.m_destPoly.push(new ClipperLib.IntPoint(this.m_srcPoly[j].X + this.m_normals[k].X * this.m_delta,
                this.m_srcPoly[j].Y + this.m_normals[k].Y * this.m_delta));
            this.m_destPoly.push(new ClipperLib.IntPoint(this.m_srcPoly[j]));
            this.m_destPoly.push(new ClipperLib.IntPoint(this.m_srcPoly[j].X + this.m_normals[j].X * this.m_delta,
